...
func run() int {
	...
	// `clyde setup` — print the hook-notification wiring on plain stdout.
	if flag.Arg(0) == "setup" {
		return runSetup(os.Stdout)
	}
				...
				if err != nil {
					...
					if urlErr != nil {
						...
						fmt.Fprintf(os.Stderr, "clyde: hook server on port %d (URL written to %s, mode 0600)\n",
							hs.Port(), hookURLPath)
						fmt.Fprintf(os.Stderr, "clyde: run `clyde setup` for the settings.json snippet (README → Hook notifications)\n")
					}
				}
...
func writeHookURLFile(url string) (string, error) {
	// Path resolution is shared with `clyde setup` (hookURLFilePath) so the
	// printed snippet can never drift from where the file actually lands.
	...
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil { //nolint:gosec // see comment
