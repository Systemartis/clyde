...
The fastest way to give us everything at once: run `clyde --crash-report` and attach the tarball it writes to your home directory — it bundles the log (`~/.cache/clyde/clyde.log`), version, and environment, nothing else. Set `CLYDE_DEBUG=1` and reproduce first if the issue needs debug-level detail.
...
- `internal/adapters` — I/O implementations: `jsonl` (session reader), `git`, `fsexplorer`, `hookserver`, `anthropicapi`, `claudesettings`, `mcpconfig`, `lspscan`, `processscan`, `systemclock`, and `tui` (the Bubble Tea program). Every adapter is constructed and injected in `cmd/clyde/main.go` — read that first.
