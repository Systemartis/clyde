...
`clyde --demo` runs against `mockdata.go` in the tui adapter — useful for golden tests, offline work, and reproducing UI bugs without needing real Claude sessions. Live mode (default) wires every adapter, starts a localhost hook server on a random port, and prints the `~/.claude/settings.json` snippet the user needs to enable hook notifications. The hook server's lifecycle is bound to the Bubble Tea program context (`hookCancel` on shutdown). When debugging hook flow, see `docs/design/hook-setup.md` (gitignored — local only).
