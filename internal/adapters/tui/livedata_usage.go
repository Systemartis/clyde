...
func deriveUsageFields(v livesession.View, d MockData) MockData {
	...
	if !fiveHRow.Empty {
		...
		fiveHRow.ResetAt = v.Reset5hAt
		fiveHRow.ResetElapsedPct = fiveHRow.Percent
	}
	d.Usage5h = fiveHRow
	...
	if !weekRow.Empty {
		...
		weekRow.ResetAt = v.ResetWeekAt
		weekRow.ResetElapsedPct = weekRow.Percent
	}
	d.UsageWeek = weekRow
...
//
// now is the CURRENT clock instant from the caller. Plan usage refreshes
// every ~5 minutes but the dashboard repaints every second; computing the
// countdown against FetchedAt froze ResetsIn/ResetElapsedPct between
// fetches. ResetsAt is absolute, so wall-clock math against now is safe.
func applyPlanUsageToMock(d MockData, pu ports.PlanUsage, err error, now time.Time) MockData {
	switch {
	...
	if pu.FiveHour.Present && !d.Usage5h.Empty {
		...
		if !pu.FiveHour.ResetsAt.IsZero() {
			...
			d.Usage5h.ResetAt = pu.FiveHour.ResetsAt
			d.Usage5h.ResetElapsedPct = windowElapsedPercent(pu.FiveHour.ResetsAt, now, 5*time.Hour)
		}
	}
	if pu.SevenDay.Present && !d.UsageWeek.Empty {
		...
		if !pu.SevenDay.ResetsAt.IsZero() {
			...
			d.UsageWeek.ResetAt = pu.SevenDay.ResetsAt
			d.UsageWeek.ResetElapsedPct = windowElapsedPercent(pu.SevenDay.ResetsAt, now, 7*24*time.Hour)
		}
	}
	return d
}
