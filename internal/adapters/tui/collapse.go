...
// ExpandedHeight returns the settled expanded-height target — the height
// the panel reaches once the expand animation finishes. Use this (not the
// animated Height) when sizing things that must match the final layout,
// e.g. viewport scroll clamping.
func (p *PanelCollapseState) ExpandedHeight() int {
	return int(p.expandedH)
}
