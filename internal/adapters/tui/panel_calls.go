...
func buildCallsContent(s Styles, p Palette, d MockData, inner, innerH int) string {
	...
	for i, grp := range d.AgentGroups {
		...
		if !grp.IsSubagent {
			...
			default:
				diffShown = renderSoloMainBlock(&sb, s, p, grp, d, inner, diffShown)
			}
			continue
		}
...
func renderSoloMainBlock(sb *strings.Builder, s Styles, p Palette, grp AgentGroup, d MockData, inner int, diffShown bool) bool {
