...
// TestSnapshot_Reset5hAnchoredToFirstEvent is the regression test for the
// "next reset pinned at 5h forever" bug: the fallback Reset5hAt anchored on
// the earliest LAST-activity among in-window sessions, so a single
// continuously-active session (LastActivity ≈ now) produced Reset5hAt ≈
// now+5h on every snapshot — the countdown never moved. The window must
// anchor on the FIRST event in the window (truncated to the hour, matching
// Anthropic's billing blocks) and tile forward.
func TestSnapshot_Reset5hAnchoredToFirstEvent(t *testing.T) {
	t.Parallel()

	now := mustTime("2026-04-30T12:00:00Z")
	sid := session.ID("sess-live")
	first := mustTime("2026-04-30T09:23:00Z") // session began 2h37m ago
	last := mustTime("2026-04-30T11:59:00Z")  // still active

	mkUsageEvt := func(id string, ts time.Time) event.Event {
		return event.NewEvent(id, ts, event.KindAssistant, string(sid), "",
			event.AssistantPayload{Usage: usagePkg.Usage{Input: 1_000}})
	}

	src := &fakeSessionSource{
		sessions: []session.Summary{{ID: sid, LastActivity: last}},
		events: map[session.ID][]event.Event{
			sid: {mkUsageEvt("ev1", first), mkUsageEvt("ev2", last)},
		},
	}
	...
	// Anchor 09:23 → truncated 09:00 → block 09:00-14:00 → reset 14:00.
	wantReset := mustTime("2026-04-30T14:00:00Z")
	if !view.Reset5hAt.Equal(wantReset) {
		t.Errorf("Reset5hAt = %v, want %v (anchored to first event, hour-truncated)", view.Reset5hAt, wantReset)
	}

	// Weekly window tiles from the same anchor: 09:00 + 7d.
	wantWeek := mustTime("2026-05-07T09:00:00Z")
	if !view.ResetWeekAt.Equal(wantWeek) {
		t.Errorf("ResetWeekAt = %v, want %v", view.ResetWeekAt, wantWeek)
	}
}
