...
func (w *windowAccumulator) add(u usage.Usage, lastActivity time.Time, age time.Duration) {
	if age > w.windowSize {
		return
	}
...
func (l *LiveSession) applyMultiWindowUsageGlobal(ctx context.Context, base View, summaries []session.Summary, now time.Time) View {
	...
	for _, ref := range refs {
		...
		u := l.usageForRef(ctx, ref, focusedID, base.TotalUsage)
		w5h.add(u, ref.LastActivity, age)
		wWeek.add(u, ref.LastActivity, age)
	}
	...
	if !w5h.earliest.IsZero() {
		base.Reset5hAt = w5h.earliest.Add(w5h.windowSize)
	}
	if !wWeek.earliest.IsZero() {
		base.ResetWeekAt = wWeek.earliest.Add(wWeek.windowSize)
	}
	return base
}
...
func (l *LiveSession) usageForRef(ctx context.Context, ref ports.GlobalSessionRef, focusedID string, focusedUsage usage.Usage) usage.Usage {
	...
	if l.usage != nil {
		l.usage.mu.Lock()
		l.usage.entries[ref.SessionID] = sessionUsageCacheEntry{usage: u, activity: ref.LastActivity}
		l.usage.mu.Unlock()
	}
	return u
}
...
func (l *LiveSession) applyMultiWindowUsageSingleProject(ctx context.Context, base View, summaries []session.Summary, now time.Time) View {
	...
	if len(summaries) > 0 {
		...
		w5h.add(base.TotalUsage, summaries[0].LastActivity, age)
		wWeek.add(base.TotalUsage, summaries[0].LastActivity, age)
	}
	...
	for _, sum := range summaries[1:] {
		...
		u := sumUsageFromEvents(evts)
		w5h.add(u, sum.LastActivity, age)
		wWeek.add(u, sum.LastActivity, age)
	}
