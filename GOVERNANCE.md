...
The project ships under the [MIT License](./LICENSE). Re-licensing or relicensing-with-exceptions requires unanimous maintainer agreement and explicit project-owner approval. We do **not** require contributors to sign a CLA — the MIT license's inbound-equals-outbound assumption (carried in repo `LICENSE` plus the contributor's signed commit) is sufficient.

## Forks

Clyde is permissively licensed. You're welcome to fork, modify, and redistribute under the MIT terms. The "Clyde" name and the visual identity are trademarks of Systemartis — see [README.md](./README.md) for the trademark notice.
